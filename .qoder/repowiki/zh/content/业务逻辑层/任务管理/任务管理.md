...
- 优化任务列表性能和用户体验
- **重构任务统计信息展示方式，将独立的TaskSummary组件替换为任务列表表格页脚的汇总统计行**
- **移除独立的TaskSummary组件，实现统计信息与任务列表的深度融合**
...
任务管理模块的UI进行了重构，将原有的独立`TaskSummary`组件替换为任务列表表格页脚的汇总统计行，实现了统计信息与任务列表的深度融合。这一变更提升了数据可见性和用户体验，使用户无需切换组件即可在任务列表中直接查看关键统计信息。

重构后的统计信息展示方式通过`TaskList.vue`组件中的`ColumnGroup`和`Row`组件实现，位于数据表格的页脚区域。统计信息分为三个主要部分：总任务数、注入状态统计和任务状态统计。总任务数显示在第一列，注入状态统计包括"存在注入"和"无注入"的数量，任务状态统计则显示"运行中"和"已完成"的任务数量。对于更详细的统计信息，用户可以通过点击省略号按钮弹出的Popover查看完整统计。

统计信息的数据来源于`task.ts`中的`taskStats`计算属性，该属性基于任务列表中的任务状态和注入状态进行实时计算。`TaskStats`接口定义了所有统计字段，包括总任务数、各状态任务数以及注入状态相关的统计。当任务列表发生变化时，`taskStats`会自动重新计算，确保统计信息的实时性和准确性。

这一重构不仅简化了UI布局，减少了组件间的跳转，还通过将统计信息与任务列表紧密结合，提高了用户的工作效率。用户可以在浏览任务的同时快速了解整体任务状态，无需额外操作即可获取关键指标。

**Section sources**
- [TaskList.vue](file://src/frontEnd/src/views/TaskList/index.vue#L124-L189) - *表格页脚统计行实现*
- [task.ts](file://src/frontEnd/src/stores/task.ts#L129-L177) - *任务统计计算属性*
- [task.ts](file://src/frontEnd/src/types/task.ts#L49-L61) - *TaskStats接口定义*
- [TaskSummary.vue](file://src/frontEnd/src/components/TaskSummary.vue) - *已移除的独立统计组件*