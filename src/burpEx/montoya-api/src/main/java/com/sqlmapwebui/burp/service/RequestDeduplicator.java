package com.sqlmapwebui.burp.service;
import com.sqlmapwebui.burp.api.HttpRequestUtils;
...
public class RequestDeduplicator {
    ...
    public static class DedupeResult {
        private final List<HttpRequestResponse> uniqueMessages;
        private final List<HttpRequestResponse> duplicateMessages;
        
        public DedupeResult(List<HttpRequestResponse> uniqueMessages, 
                           List<HttpRequestResponse> duplicateMessages) {
            this.uniqueMessages = uniqueMessages;
            this.duplicateMessages = duplicateMessages;
        }
        
        public List<HttpRequestResponse> getUniqueMessages() {
            return uniqueMessages;
        }
        
        public List<HttpRequestResponse> getDuplicateMessages() {
     ...
     * @return 去重结果
     */
    public static DedupeResult deduplicate(List<HttpRequestResponse> messages) {
        List<HttpRequestResponse> uniqueMessages = new ArrayList<>();
        List<HttpRequestResponse> duplicateMessages = new ArrayList<>();
        Set<String> seenFingerprints = new HashSet<>();
        
        for (HttpRequestResponse message : messages) {
            String fingerprint = generateFingerprint(message.request());
     ...
     */
    public static String generateFingerprint(HttpRequest request) {
     ...
     * 获取请求的简短描述（用于日志）
     */
    public static String getRequestDescription(HttpRequest request) {
        try {
            URL url = new URL(request.url());
            return request.method() + " " + url.getHost() + url.getPath();
        } catch (Exception e) {
