...
- [src/backEnd/start.sh](file://src/backEnd/start.sh)
- [src/backEnd/start.bat](file://src/backEnd/start.bat)
- [src/backEnd/prepare_offline.sh](file://src/backEnd/prepare_offline.sh)
- [src/backEnd/prepare_offline.bat](file://src/backEnd/prepare_offline.bat)
- [src/backEnd/startup.conf](file://src/backEnd/startup.conf)
</cite>

## 更新摘要
**变更内容**
- 新增了启动脚本（start.sh、start.bat）的详细说明
- 增加了离线部署流程的完整文档
- 新增了统一的启动配置文件 startup.conf 的配置说明
- 更新了部署架构图以反映新的启动流程
- 扩展了故障排查指南以包含新的启动模式
...
## 简介
本文件面向运维与开发团队，提供 sqlmapWebUI 在不同环境（开发、测试、生产）下的完整部署配置说明。重点解析后端配置文件与环境变量，解释 .env.example 的使用方法与最佳实践；深入说明 Uvicorn 服务器配置项（uvicorn_config.json）；给出前后端分离部署、反向代理与负载均衡的推荐架构；并提供配置验证步骤与常见错误排查方法，帮助快速、稳定地交付系统。本次更新特别增加了新的启动脚本、配置文件和离线部署流程的详细说明。
...
- 启动脚本与配置
  - Linux/macOS 启动脚本：src/backEnd/start.sh
  - Windows 启动脚本：src/backEnd/start.bat
  - 离线依赖准备脚本：src/backEnd/prepare_offline.sh 和 prepare_offline.bat
  - 统一启动配置文件：src/backEnd/startup.conf
- 前端环境变量
...
- [src/backEnd/start.sh](file://src/backEnd/start.sh#L1-L310)
- [src/backEnd/start.bat](file://src/backEnd/start.bat#L1-L306)
- [src/backEnd/prepare_offline.sh](file://src/backEnd/prepare_offline.sh#L1-L154)
- [src/backEnd/prepare_offline.bat](file://src/backEnd/prepare_offline.bat#L1-L148)
...
- 启动脚本（start.sh/start.bat）
  - 跨平台支持：提供 Linux/macOS 和 Windows 两个版本
  - 自动化流程：自动检测 Python 版本、创建虚拟环境、安装依赖、启动服务
  - 网络模式：支持 online、intranet、offline 三种网络模式
  - 镜像配置：内置多个国内 PyPI 镜像源（清华、阿里、中科大等）
  - 错误处理：完善的错误检测与提示机制
- 统一配置文件（startup.conf）
  - 集中管理：所有启动相关配置集中在一个文件中
  - 网络模式：NETWORK_MODE=online|intranet|offline
  - 镜像配置：PYPI_MIRROR 指定公网镜像，PRIVATE_MIRROR_URL 指定私域镜像
  - 服务配置：HOST、PORT、ADMIN_USERNAME、ADMIN_PASSWORD
  - 性能优化：SKIP_DEPS_CHECK 可跳过依赖检查以加快启动速度
- 离线部署流程
  - 准备阶段：在有网络的机器上运行 prepare_offline.sh/prepare_offline.bat
  - 依赖安装：脚本会创建虚拟环境并安装所有依赖
  - 离线包下载：可选下载离线包到 offline_packages 目录
  - 部署阶段：将整个 backEnd 目录复制到离线机器
  - 配置修改：设置 NETWORK_MODE=offline
  - 启动服务：运行 start.sh/start.bat
...
- 启动脚本问题
  - Python 未找到：确保系统已安装 Python 3.10+
  - 依赖安装失败：检查网络连接或更换镜像源
  - 虚拟环境创建失败：检查磁盘空间和权限
  - 离线模式问题：确保已正确设置 NETWORK_MODE=offline
...
## 结论
通过规范的环境变量管理、严格的 CORS 与日志配置、合理的前后端分离与反向代理部署，sqlmapWebUI 可在开发、测试与生产环境中稳定运行。建议在生产环境收紧安全配置、启用 HTTPS、使用容器化与反向代理，并建立完善的监控与日志采集体系。新增的启动脚本和离线部署流程大大简化了部署复杂度，提高了部署效率和可靠性。
