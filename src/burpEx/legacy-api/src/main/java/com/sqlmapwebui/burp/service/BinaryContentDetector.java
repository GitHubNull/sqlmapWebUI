...
public class BinaryContentDetector {
     ...
     * @return 检测结果
     */
    public static DetectionResult detect(IHttpRequestResponse requestResponse, IExtensionHelpers helpers) {
        if (requestResponse == null || helpers == null) {
            return new DetectionResult(true, "请求对象或帮助器为空");
        }
        
        byte[] request = requestResponse.getRequest();
        if (request == null || request.length == 0) {
            return new DetectionResult(true, "请求数据为空");
        }
    ...
    private static String getContentType(IRequestInfo requestInfo) {
        ...
        for (String header : headers) {
            if (header.toLowerCase().startsWith("content-type:")) {
                String value = header.substring("content-type:".length()).trim();
                // 去除charset等参数
     ...
     * 便捷方法：直接判断请求是否为二进制
     */
    public static boolean isBinaryRequest(IHttpRequestResponse requestResponse, IExtensionHelpers helpers) {
        return detect(requestResponse, helpers).isBinary();
    }
    
    /**
     * 便捷方法：直接判断请求是否为纯文本
     */
    public static boolean isTextRequest(IHttpRequestResponse requestResponse, IExtensionHelpers helpers) {
        return detect(requestResponse, helpers).isTextContent();
    }
}
