...
- [7. 扩展插件使用](#7-扩展插件使用)
- [8. 高级功能](#8-高级功能)
  - [8.1 批量导入HTTP请求](#81-批量导入http请求)
  - [8.2 自定义 SQLMap 参数](#82-自定义-sqlmap-参数)
  - [8.3 引导式参数编辑器](#83-引导式参数编辑器)
  - [8.4 批量请求头导入](#84-批量请求头导入)
  - [8.5 自动刷新间隔配置](#85-自动刷新间隔配置)
  - [8.6 215个SQLMap参数完整支持](#86-215个sqlmap参数完整支持)
- [9. 常见问题](#9-常见问题)
...
系统日志查看器方便排查问题和监控系统状态：

#### 日志类型
- **应用日志**: 应用程序运行日志，包含扫描任务执行记录
- **访问日志**: HTTP请求访问记录
- **错误日志**: 系统错误和异常记录

#### 功能特性
- **类型切换**: 支持三种日志类型快速切换
- **行数设置**: 自定义显示行数（50/100/200/500行）
- **实时刷新**: 查看最新日志内容
- **暗色主题**: 适配暗色模式

#### 使用方式
1. 在配置页面找到「系统日志查看器」入口
2. 选择要查看的日志类型
3. 设置显示行数
4. 点击刷新获取最新日志
...
- **排序功能**: 支持按ID、创建时间等字段排序
- **分页功能**: 支持分页浏览，可调整每页数量
- **ID标识**: 配置卡片显示ID标识，便于识别
- **自动刷新**: Burp插件创建任务后自动刷新表格
...
**v1.8.33+ 改进**: 命令行预览组件采用 GitHub Dark 主题风格，提供更好的可读性。
...
AddTask页面采用模块化组件设计：
- **HTTP请求编辑器**: 独立的代码编辑器组件，支持语法高亮
- **参数配置面板**: 引导式参数选择界面
- **命令行预览**: 实时预览生成的SQLMap命令
- **扫描配置选择**: 快速选择默认/常用/历史配置
...
**v1.8.33+ 改进**: 配置页面自动刷新间隔滑块添加刻度标记，更直观的选择体验。

### 8.6 215个SQLMap参数完整支持

本项目支持SQLMap的**215个参数**（除`-r`外），完全兼容SQLMap 1.9.11.3+：

| 分类 | 数量 | 说明 |
|------|------|------|
| Target | 8 | 目标定义（URL、日志、批量文件等） |
| Request | 51 | HTTP请求配置（认证、代理、CSRF等） |
| Optimization | 5 | 性能优化（线程、连接等） |
| Injection | 17 | 注入测试配置（测试参数、注入技术等） |
| Detection | 8 | 检测配置（level、risk、匹配规则等） |
| Techniques | 9 | 注入技术配置（UNION、DNS外泄等） |
| Enumeration | 36 | 数据枚举（表、列、用户等） |
| OS Takeover | 8 | 操作系统接管（命令执行、shell等） |
