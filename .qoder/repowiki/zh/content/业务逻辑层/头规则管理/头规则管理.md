...
## 更新摘要
**变更内容**   
- 新增作用域匹配功能，支持多维度（协议、主机、IP、端口、路径）的规则作用域控制
- 批量导入功能增强，支持作用域配置的批量创建
- 会话头管理功能扩展，支持作用域配置和目标URL匹配
- 更新规则匹配引擎，集成作用域匹配逻辑
- 新增作用域匹配流程时序图
...
规则创建过程中使用了事务处理，确保数据的一致性。每个规则包含名称、请求头名称、请求头值、替换策略、匹配条件、优先级、启用状态和作用域配置等属性。作用域配置通过`HeaderScope`模型实现，支持协议、主机名、IP地址、端口和路径五个维度的匹配。
...
读取操作支持按优先级降序排序，确保高优先级规则优先处理。查询结果包含规则的完整信息，并可选择只返回启用状态的规则。作用域配置以JSON格式存储在数据库的`scope_config`列中，读取时会自动反序列化为`HeaderScope`对象。
...
更新操作支持部分字段更新，未提供的字段保持原值不变。系统还会检查新规则名称是否与其他规则冲突。当更新作用域配置时，系统会将`HeaderScope`对象序列化为JSON字符串后存储。
...
会话头在设置时会同时持久化到数据库，确保服务重启后仍能恢复部分状态。每个会话头可配置作用域，只有当目标URL匹配作用域配置时，该会话头才会被应用。
...
### 作用域控制
规则的作用域通过以下方式控制：
- 持久化规则：支持全局作用域和基于`HeaderScope`的作用域
- 会话级规则：基于客户端IP的作用域，只对特定客户端生效
- 匹配条件：通过正则表达式匹配请求头值，实现条件性应用
- 作用域匹配：通过`ScopeMatcher`类实现，支持协议、主机名、IP、端口、路径五个维度的匹配

`ScopeMatcher`的匹配逻辑如下：
- `scope`为`None`时，表示全局生效，返回`True`
- `scope.is_empty()`为`True`时，表示全局生效，返回`True`
- 所有配置的维度必须同时匹配（AND逻辑）
- 各维度支持关键字匹配（默认）和正则表达式匹配两种模式
- 支持通配符（*）匹配和多值（逗号分隔）匹配
...
- [scope_matcher.py](file://src/backEnd/utils/scope_matcher.py#L19-L141)
...
批量操作采用部分成功策略，即使某些项目失败，其他成功项目仍会被创建。批量创建支持为所有规则配置统一的作用域，通过`ParsedHeaderBatchCreateRequest`中的`rule_config`参数传递。
...
应用过程遵循UPSERT语义：如果请求头已存在则替换，如果不存在则新增。会话头支持作用域配置，只有当目标URL与作用域匹配时，该会话头才会被应用。
...
- [header_processor.py](file://src/backEnd/utils/header_processor.py#L10-L241)
- [scope_matcher.py](file://src/backEnd/utils/scope_matcher.py#L36-L141)
...
- **启用状态**：布尔值，控制规则是否生效
- **作用域配置**：`HeaderScope`对象，支持协议、主机名、IP、端口、路径五个维度的匹配
...
7. **缓存机制**：`ScopeMatcher`使用URL解析缓存和正则表达式编译缓存，提高匹配效率
