# 任务详情页

<cite>
**本文档引用的文件**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue)
- [task.ts](file://src/frontEnd/src/api/task.ts)
- [taskService.py](file://src/backEnd/service/taskService.py)
- [Task.py](file://src/backEnd/model/Task.py)
- [taskStatus.py](file://src/backEnd/model/TaskStatus.py)
- [taskStore.ts](file://src/frontEnd/src/stores/task.ts)
- [task.ts](file://src/frontEnd/src/types/task.ts)
</cite>

## 目录
1. [简介](#简介)
2. [页面布局](#页面布局)
3. [数据并行加载策略](#数据并行加载策略)
4. [信息模块数据获取API](#信息模块数据获取api)
5. [前端组件结构](#前端组件结构)
6. [用户交互设计](#用户交互设计)

## 简介
任务详情页是SQLMapWebUI系统中的核心功能模块，用于展示单个扫描任务的详细信息。该页面提供了全面的任务信息视图，包括基础信息、HTTP请求信息、扫描配置、扫描结果、任务日志和错误记录等关键模块。通过该页面，用户可以深入了解任务的执行状态、配置参数和扫描结果，为安全测试提供决策支持。

**Section sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue)

## 页面布局
任务详情页采用响应式设计，主要由页面头部和详情内容两大部分组成。页面头部包含返回列表按钮和操作按钮组，详情内容部分通过卡片式布局展示各个信息模块。

页面采用Flexbox和Grid布局技术，确保在不同屏幕尺寸下的良好显示效果。在桌面端，信息模块采用多列网格布局；在移动端，自动切换为单列布局。每个信息模块都以卡片形式呈现，包含图标、标题和内容区域，通过CSS变量实现主题化设计。

主要布局组件包括：
- 页面头部（page-header）：包含导航和操作按钮
- 信息卡片（info-card）：封装各个信息模块
- 信息网格（info-grid）：用于基础信息的网格布局
- 加载容器（loading-container）：显示加载状态
- 内容区域（detail-content）：包含所有信息模块

**Section sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue#L499-L579)

## 数据并行加载策略
任务详情页采用并行加载策略来提高数据获取效率。当页面加载时，系统会同时发起多个异步请求来获取不同模块的数据，而不是按顺序逐个加载。

在`loadTaskDetail`函数中，通过`Promise.all`方法并行加载HTTP请求信息、载荷详情、任务日志和错误记录等数据。这种并行加载方式显著减少了页面的总体加载时间，提升了用户体验。

```mermaid
flowchart TD
Start([页面加载]) --> LoadBasic["加载任务基本信息"]
LoadBasic --> ParallelLoad["并行加载其他数据"]
ParallelLoad --> LoadHttp["加载HTTP请求信息"]
ParallelLoad --> LoadPayload["加载载荷详情"]
ParallelLoad --> LoadLogs["加载任务日志"]
ParallelLoad --> LoadErrors["加载错误记录"]
LoadHttp --> Wait["等待所有请求完成"]
LoadPayload --> Wait
LoadLogs --> Wait
LoadErrors --> Wait
Wait --> Display["显示完整页面"]
```

**Diagram sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue#L448-L507)

**Section sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue#L448-L507)

## 信息模块数据获取API
任务详情页通过多个API端点获取不同信息模块的数据。这些API由后端服务提供，前端通过封装的API函数进行调用。

### HTTP请求信息获取
获取任务的HTTP请求信息，包括请求方法、请求头和请求体。

```mermaid
sequenceDiagram
participant 前端 as 前端
participant API as API接口
participant 服务 as TaskService
participant 任务 as Task
前端->>API : getHttpRequestInfo(taskId)
API->>服务 : get_task_http_request_info(taskId)
服务->>任务 : 获取任务对象
任务-->>服务 : 返回任务信息
服务-->>API : 构造HTTP信息
API-->>前端 : 返回HTTP请求信息
```

**Diagram sources**
- [task.ts](file://src/frontEnd/src/api/task.ts)
- [taskService.py](file://src/backEnd/service/taskService.py)

### 扫描配置获取
获取任务的扫描配置参数，如检测级别、风险级别、注入技术等。

```mermaid
sequenceDiagram
participant 前端 as 前端
participant API as API接口
participant 服务 as TaskService
前端->>API : getScanOptions(taskId)
API->>服务 : get_task_scan_options(taskId)
服务-->>API : 返回扫描配置
API-->>前端 : 返回配置信息
```

**Diagram sources**
- [task.ts](file://src/frontEnd/src/api/task.ts)
- [taskService.py](file://src/backEnd/service/taskService.py)

### 载荷详情获取
获取任务的扫描载荷详情，包括状态、内容类型和载荷内容。

```mermaid
sequenceDiagram
participant 前端 as 前端
participant API as API接口
participant 服务 as TaskService
前端->>API : getPayloadDetail(taskId)
API->>服务 : get_payload_detail_by_task_id(taskId)
服务-->>API : 返回载荷详情
API-->>前端 : 返回载荷数据
```

**Diagram sources**
- [task.ts](file://src/frontEnd/src/api/task.ts)
- [taskService.py](file://src/backEnd/service/taskService.py)

### 任务日志获取
获取任务的执行日志，用于跟踪任务的执行过程。

```mermaid
sequenceDiagram
participant 前端 as 前端
participant API as API接口
participant 服务 as TaskService
前端->>API : getTaskLogs(taskId)
API->>服务 : find_task_log_by_taskid(taskId)
服务-->>API : 返回日志列表
API-->>前端 : 返回日志数据
```

**Diagram sources**
- [task.ts](file://src/frontEnd/src/api/task.ts)
- [taskService.py](file://src/backEnd/service/taskService.py)

### 错误记录获取
获取任务的错误记录，帮助用户诊断问题。

```mermaid
sequenceDiagram
participant 前端 as 前端
participant API as API接口
participant 服务 as TaskService
前端->>API : getErrors(taskId)
API->>服务 : get_task_errors_by_taskId(taskId)
服务-->>API : 返回错误列表
API-->>前端 : 返回错误数据
```

**Diagram sources**
- [task.ts](file://src/frontEnd/src/api/task.ts)
- [taskService.py](file://src/backEnd/service/taskService.py)

**Section sources**
- [task.ts](file://src/frontEnd/src/api/task.ts)
- [taskService.py](file://src/backEnd/service/taskService.py)

## 前端组件结构
任务详情页的前端组件基于Vue 3和TypeScript构建，采用组合式API（Composition API）设计模式。组件结构清晰，职责分明，便于维护和扩展。

### 主要组件构成
- **页面容器组件**：负责整体布局和导航
- **信息卡片组件**：封装各个信息模块的显示逻辑
- **数据加载组件**：处理异步数据获取和状态管理
- **交互按钮组件**：提供用户操作接口

### 组件依赖关系
```mermaid
classDiagram
class TaskDetailView {
+task : Task | null
+loading : boolean
+error : string
+loadTaskDetail() : Promise~void~
+refreshData() : Promise~void~
+handleStopTask() : void
+handleDeleteTask() : void
}
class TaskStore {
+taskList : Task[]
+currentTask : Task | null
+fetchTaskList() : Promise~void~
+deleteTask(taskId : string) : Promise~void~
+stopTask(taskId : string) : Promise~void~
}
class TaskApi {
+getTaskList() : Promise~Task[]~
+getHttpRequestInfo(taskId : string) : Promise~any~
+getPayloadDetail(taskId : string) : Promise~any~
+getTaskLogs(taskId : string) : Promise~string[]~
+getErrors(taskId : string) : Promise~any[]~
}
class Task {
+taskid : string
+engineid : number
+scanUrl : string
+host : string
+status : TaskStatus
+createTime : string
+headers? : string[]
+body? : string
+options? : TaskOptions
}
class TaskOptions {
+level? : number
+risk? : number
+technique? : string
+dbms? : string
+threads? : number
}
TaskDetailView --> TaskStore : "使用"
TaskDetailView --> TaskApi : "调用"
TaskDetailView --> Task : "显示"
TaskStore --> Task : "管理"
```

**Diagram sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue)
- [taskStore.ts](file://src/frontEnd/src/stores/task.ts)
- [task.ts](file://src/frontEnd/src/types/task.ts)

**Section sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue)
- [taskStore.ts](file://src/frontEnd/src/stores/task.ts)

## 用户交互设计
任务详情页提供了丰富的用户交互功能，包括数据刷新、任务停止和任务删除等操作。所有交互操作都配有确认对话框，防止误操作。

### 主要交互功能
1. **返回列表**：点击返回按钮可导航回任务列表页面
2. **刷新数据**：点击刷新按钮重新加载任务详情
3. **停止任务**：对于正在运行的任务，可点击停止按钮终止扫描
4. **删除任务**：可删除已完成或已停止的任务

### 交互流程
```mermaid
flowchart TD
A([用户操作]) --> B{操作类型}
B --> |刷新数据| C["调用refreshData()"]
C --> D["并行加载所有数据"]
D --> E["显示成功提示"]
B --> |停止任务| F["显示确认对话框"]
F --> G{用户确认?}
G --> |是| H["调用stopTask()"]
H --> I["更新任务状态"]
I --> J["显示成功提示"]
G --> |否| K["取消操作"]
B --> |删除任务| L["显示确认对话框"]
L --> M{用户确认?}
M --> |是| N["调用deleteTask()"]
N --> O["从列表移除任务"]
O --> P["跳转到任务列表"]
M --> |否| Q["取消操作"]
```

**Diagram sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue#L448-L507)

**Section sources**
- [TaskDetail/index.vue](file://src/frontEnd/src/views/TaskDetail/index.vue#L448-L507)