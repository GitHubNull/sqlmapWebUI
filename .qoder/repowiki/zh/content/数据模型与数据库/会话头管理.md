...
`SessionHeader` 类定义了会话性请求头的数据结构，基于 Pydantic 模型实现，确保数据的完整性和有效性。
...
**Diagram sources**  
- [SessionHeader.py](file://src/backEnd/model/SessionHeader.py#L5-L61)
...
`SessionHeaderManager` 类实现了会话头的完整生命周期管理，采用内存存储结合数据库持久化的双重机制。
...
会话头与持久化头规则通过 `HeaderProcessor` 协同工作，形成完整的请求头处理流程。
...
会话头的生命周期由创建、读取、更新、删除（CRUD）和自动清理组成。
...
### 读取逻辑
- `get_session_headers()`: 仅返回未过期的会话头
...
### 数据库持久化
- **表名**: `session_headers`
- **字段**: client_ip, header_name, header_value, priority, expires_at, created_at
...
# 批量设置会话头
headers = [
    SessionHeaderCreate(header_name="A", header_value="1", ttl=300),
    SessionHeaderCreate(header_name="B", header_value="2", ttl=600),
]
